...
class ApiKeyWebTests(unittest.TestCase):
    ...
    @patch("src.settings.web.verify_auth_token", return_value=VERIFIED_CLAIMS)
    ...
    def test_create_api_key_uses_authenticated_subject(self, service_type, _verify):
        ...
        identity = service_type.return_value.create.call_args.args[0]
        self.assertEqual(identity.subject_id, "auth-user-1")
        self.assertEqual(identity.tenant_id, "ten_9664c024babc4110")
        self.assertEqual(service_type.return_value.create.call_args.args[1:], ("codex", 30))
    ...
    def test_revoke_is_scoped_to_authenticated_subject(self, service_type, _verify):
        ...
        identity = service_type.return_value.revoke.call_args.args[0]
        self.assertEqual(identity.subject_id, "auth-user-1")
        self.assertEqual(service_type.return_value.revoke.call_args.args[1], "key-1")
