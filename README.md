...
MCP client / Browser
        │
        ├─ mcp.lynply.com/mcp ── MCPAuthMiddleware ── 22 MCP tools
        │
...
기한이 된 항목을 한 문제씩 제시하고 답변을 판정한 뒤 기록합니다.
...
공개 MCP 계약은 22개 도구 이름과 입력 필드로 테스트에 고정되어 있습니다.
...
| `plan_learning_feedback` | `assessment` | `confidence=medium`, `missing_concepts`, `misconceptions`, `evidence_refs`, `hint`, `next_question` |
...
| `record_learning_attempt` | `session_id`, `question_id`, `answer`, `assessment`, `confidence`, `feedback_plan` | `missing_concepts`, `misconceptions`, `evidence_refs`, `next_question`, `next_question_type=retrieval`, `next_evidence_refs`, `client_request_id` |
...
6. 학습은 답변 전 근거를 노출하지 않고, 판정은 evidence 기반 client LLM 평가로 기록합니다.
7. 학습 결과의 Knowledge 승격은 후보별 사용자 승인 후에만 수행합니다.
8. 기준본 준비·확정·업무 적용·후속 버전 선택은 모두 사용자의 명시적 요청으로만 수행합니다.
