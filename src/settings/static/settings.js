...
async function loadSettings() {
  try {
    ...
    byId("access-key-state").textContent = data.s3_access_key_configured ? "등록됨" : "미등록";
    byId("secret-key-state").textContent = data.s3_secret_key_configured ? "등록됨" : "미등록";
    byId("updated-at").textContent = data.updated_at ? new Date(data.updated_at).toLocaleString("ko-KR") : "—";
    message("");
  } catch (error) {
